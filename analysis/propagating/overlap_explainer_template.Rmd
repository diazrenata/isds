...
set.seed(as.integer(params$supplied_seed))
...
ls_abunds_up <- ceiling(ls_abunds * 200 / min(ls_abunds))
...
```{r ecdf plot demo, echo = F, fig.dim = c(6,2)}
ls_up_error_plots_ecdf <- lapply(ls_up_error_communities, FUN = plot_pipeline, overlap_type = "ecdf")
for(i in 1:length(ls_up_error_plots_ecdf)) {
  
  suppressWarnings(gridExtra::grid.arrange(grobs = ls_up_error_plots_ecdf[[i]], nrow = 1, top = grid::textGrob(paste0(names(scenarios)[i], " + logseries SAD + upsampled N based on logseries means"),gp=grid::gpar(fontsize=12))))
}
```
