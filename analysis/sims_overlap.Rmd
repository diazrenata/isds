...
sims <- read.csv(here::here("analysis", "sims.csv"), stringsAsFactors = F)

sims <- filter(sims, sim == 1)
  
sims_spassign <- filter(sims, source == "constrained") %>%
  select(time_chunk, species)

for(i in unique(sims$time_chunk)) {
  
  this_spvect <- filter(sims_spassign, time_chunk == i)$species
  
  sims$species[ which((sims$time_chunk == i), (sims$source == "uniform"))] <- this_spvect
  
  
}
  
sims <- sims %>%
mutate(source = paste0(source, time_chunk)) %>%
select(species, wgt, source)

sources <- as.list(unique(sims$source))

scenario_communities <- lapply(sources, FUN = function(this_source, sim_df) return(sim_df %>% filter(source == this_source) %>% select(-source)), sim_df =sims)

names(scenario_communities) <- unlist(sources)

scenario_plots <- lapply(scenario_communities, FUN = plot_pipeline)
```

```{r show plots, echo = F, fig.dim = c(12,4)}
for(i in 1:length(scenario_plots)) {
  
  suppressWarnings(gridExtra::grid.arrange(grobs = scenario_plots[[i]], nrow = 1, top = grid::textGrob(names(scenario_communities)[i],gp=grid::gpar(fontsize=12))))
}
...
new_means <- lapply(scenario_communities, FUN = function(community_df) 
  return((community_df %>% group_by(species) %>% summarize(meanwgt = mean(wgt)) %>% ungroup())$meanwgt))

up_communities <- lapply(new_means, FUN = function(means_vect)
  return(sample_community(means = means_vect, abunds = rep(100, times = length(means_vect)))))

up_plots <- lapply(up_communities, FUN = plot_pipeline)

```
Here's the outcomes:

```{r show error prop outcomes, echo = F, fig.dim = c(12,4)}
for(i in 1:length(up_plots)) {
  
  suppressWarnings(gridExtra::grid.arrange(grobs = up_plots[[i]], nrow = 1, top = grid::textGrob(paste0(names(scenario_communities)[i], " + upsampled N"),gp=grid::gpar(fontsize=12))))
}
```
