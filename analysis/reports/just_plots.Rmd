...
for(i in 1:length(all_plots)) {
  ...
  stdevs <- unique(all_plots[[i]]$metadata$stdev)[ which(unique(all_plots[[i]]$metadata$stdev) != "NA")]
  ...
  for(j in 1:length(stdevs)) {
    ...
    these_plots[[k + 2]] <- ggplot(data = these_results, aes(x = width, y = mean_p)) +
      geom_point() + 
      ylim(0, 1) +
      theme_bw() +
      geom_point(data = empirical_results, aes(x = width, y = mean_p), color = "red") +
      geom_hline(yintercept = empirical_results$mean_p, color = "red") +
      geom_vline(xintercept = empirical_results$width, color = "red") +
