...
```{r fit gaussians to bsds, warning=FALSE}
library(mclust)
...
bsd_clumps <- bsd_dat %>%
  group_by(source) %>%
  summarize(AICc_nbclumps = get_n_modes(meanwgt, max_nb_clumps = 5, measurer = "aicc"),
         BIC_nbclumps = get_n_modes(meanwgt, max_nb_clumps = 5, measurer = "bic")) %>%
  ungroup()
...
```{r fit gaussians to isds, warning=FALSE}
library(mclust)
...
```{r minimally constrained uniform, warning=FALSE}
...
```{r nind btwn minima, warning=FALSE}

isd_nindpermode <- lapply(as.matrix(unique(isd_dat$source)), FUN = function(a_source, isd_dat) 
  return(get_nind_permode(size_vector = filter(isd_dat, source == a_source)$wgt)), isd_dat = isd_dat) 
names(isd_nindpermode) <- unique(isd_dat$source)
  
isd_nindpermode <- isd_nindpermode %>%
  bind_rows(.id = "source")
